...
<?PHP
    ...
    if (mysqli_query($con, $sql)) {
        echo "Table Amazon created successfully"."<br>";
    } else {
        echo "Error creating table: " . mysqli_error($con);
    }
    $del="DELETE FROM Amazon";
    ...
    while($i<365){
        ...
        $sql= mysqli_query($con,"INSERT INTO Amazon(Stocks, Open, High, Low, Close, Volume, Date)
                           ...
                           while($i<365){
                           $sql= mysqli_query($con,"UPDATE Amazon SET Open='{$arrprice[$i][1]}', High='{$arrprice[$i][2]}',Low='{$arrprice[$i][3]}', Close='{$arrprice[$i][4]}', Volume='{$arrprice[$i][5]}', Date='{$arrprice[$i][0]}'
                                              WHERE ID='$i'");
                                              $i++;
                                              $date++;
                                              if($date==30)$date=1;

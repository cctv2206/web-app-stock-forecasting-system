...
<?PHP
    ...
    $sql = "CREATE TABLE IF NOT EXISTS GE
    (
    ...
    if (mysqli_query($con, $sql)) {
        echo "Table GE created successfully"."<br>";
    } else {
    ...
    while($i<365){
        ...
        $sql= mysqli_query($con,"INSERT INTO GE(Stocks, Open, High, Low, Close, Volume, Date)
                           ...
                           while($i<365){
                           $sql= mysqli_query($con,"UPDATE GE SET Open='{$arrprice[$i][1]}', High='{$arrprice[$i]  [2]}',Low='{$arrprice[$i][3]}', Close='{$arrprice[$i][4]}', Volume='{$arrprice[$i][5]}', Date='{$arrprice[$i][0]}'
                                              WHERE ID='$i'");
                                              $i++;
                                              $date++;
                                              if($date==30)$date=1;

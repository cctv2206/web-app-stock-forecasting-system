...
<?PHP
    ...
    if (mysqli_query($con, $sql)) {
        echo "Table Google created successfully"."<br>";
    } else {
        echo "Error creating table: " . mysqli_error($con);
    }
    $del="DELETE FROM Google";
    ...
    while($i<365){
        $sql= mysqli_query($con,"INSERT INTO Google(Stocks, Open, High, Low, Close, Volume, Date)
VALUES ('$arr','{$arrprice[$i][1]}','{$arrprice[$i][2]}','{$arrprice[$i][3]}','{$arrprice[$i][4]}','{$arrprice[$i][5]}','{$arrprice[$i][0]}')");
                           ...
                           while($i<365){
                           $sql= mysqli_query($con,"UPDATE Google SET Open='{$arrprice[$i][1]}', High='{$arrprice[$i]  [2]}',Low='{$arrprice[$i][3]}', Close='{$arrprice[$i][4]}', Volume='{$arrprice[$i][5]}', Date='{$arrprice[$i][0]}'
                                              WHERE ID='$i'");
                                              $i++;
                                              $date++;
                                              if($date==30)$date=1;

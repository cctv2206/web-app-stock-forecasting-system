...
<?PHP
    ...
    $sql = "CREATE TABLE IF NOT EXISTS SIRI
    (
    ...
    if (mysqli_query($con, $sql)) {
        echo "Table SIRI created successfully"."<br>";
    } else {
        echo "Error creating table: " . mysqli_error($con);
    }
    $del="DELETE FROM SIRI";
    ...
    while($i<365){
        ...
        $sql= mysqli_query($con,"INSERT INTO SIRI(Stocks, Open, High, Low, Close, Volume, Date)
                           ...
                           while($i<365){
                           $sql= mysqli_query($con,"UPDATE SIRI SET Open='{$arrprice[$i][1]}', High='{$arrprice[$i]  [2]}',Low='{$arrprice[$i][3]}', Close='{$arrprice[$i][4]}', Volume='{$arrprice[$i][5]}', Date='{$arrprice[$i][0]}'
                                              WHERE ID='$i'");
                                              $i++;
                                              $date++;
                                              if($date==30)$date=1;
